...
export function ResultsGrid() {
  ...
  const canZip = selected.length > 0 ? selected.length >= 2 : results.length >= 2;

  const downloadZip = async () => {
    if (!canZip) {
      toast({
        title: "Selecione mais mockups",
        description: "É necessário pelo menos dois mockups para exportar o ZIP.",
      });
      return;
    }
